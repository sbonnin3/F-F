<!-- <template>
  <div class="ticket-purchase">
    <h1>Achat de Tickets pour l'Événement Automobile</h1>
    <div class="cards">
      <div class="card" v-for="type in ticketTypes" :key="type.value" @click="selectTicket(type.value)">
        <div class="card-content">
          <h2>{{ type.label }}</h2>
          <p>{{ type.description }}</p>
          <p>Prix: {{ type.price }}€/personne</p>
        </div>
      </div>
    </div>
    <div class="form">
      ...
      <div class="total-price">Prix Total: {{ totalPrice }}€</div>
      <button class="purchase-btn" @click="handlePurchase">Acheter</button>
    </div>
    <div class="confirmation" v-if="purchaseConfirmed">Réservation Confirmée</div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ticketTypes: [
        { value: 'saturday', label: 'Samedi', price: 50, description: 'Une journée pleine d\'action!', image: 'saturday.jpg' },
        { value: 'sunday', label: 'Dimanche', price: 50, description: 'Profitez des finales!', image: 'sunday.jpg' },
        { value: 'both', label: 'Weekend Complet', price: 85, description: 'Le forfait complet pour les vrais fans!', image: 'weekend.jpg' }
      ],
      ticketType: '',
      numberOfPeople: 1,
  ...
  computed: {
    totalPrice() {
      const selectedTicket = this.ticketTypes.find(ticket => ticket.value === this.ticketType);
      return selectedTicket ? selectedTicket.price * this.numberOfPeople : 0;
    },
  },
  methods: {
    selectTicket(type) {
    ...
    handlePurchase() {
      if (this.ticketType && this.numberOfPeople > 0) {
        this.purchaseConfirmed = true;
        // Intégration avec le système de paiement et gestion des données
      }
    },
  },
};
